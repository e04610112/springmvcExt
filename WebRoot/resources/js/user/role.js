...
role = {
    ...
    del: ctx + "/role/del/",
    //删除
    allModules: ctx + "/module",
    //系统菜单
    saveModules: ctx + "/module/saverole",
    //保存角色与系统菜单
...
role.pageSize = parseInt(role.pageSizeCombo.getValue()); /** 基本信息-数据源 */
...
role.tbar = [role.addAction, '-', role.editAction, '-', role.deleteAction, '-', role.searchField]; /** 底部工具条 */
                    ...
                    callback: function (json) {
                        ...
                        if (json.length > 0) {
                            for (var i = 0; i < json.length; i++) {
                                ...
                                if (treeNode) treeNode.getUI().checkbox.checked = true;
                            }
                        }
                    }
                });
...
role.addWindow = new Ext.Window({
    ...
    buttons: [{
        ...
        handler: function () {
            ...
            form.reset();
            if (id != '') form.findField("roleId").setValue(id);
        }
    }]
});
