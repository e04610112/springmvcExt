...
module = {
	all : ctx + '/module/all.do',// 加载所有
	save : ctx + "/module/save.do",//保存
	del : ctx + "/module/del.do",//删除
	MODULEMAP :eval('(${moduleMap})'),
	pageSize : 20, // 每页显示的记录数
	//LEAF :  eval('(${fieldsMaps})')["leaf"],
	//fieldsMap : eval('(${fieldsMaps})'),
	//EXPANDED : eval('(${fieldsMaps})')["expanded"],
	//ISDISPLAY:eval('(${fieldsMaps})')["isdisplay"]
...
module.delFun = function() {
	...
	Ext.Msg.confirm('提示', '你真的要删除选中菜单及其包含的所有子菜单吗?', function(btn, text) {
				if (btn == 'yes') {
					// 发送请求
					Share.AjaxRequest({
								url : module.del +"?moduleId="+record.data.moduleId,
								callback : function(json) {
									module.alwaysFun();
									module.store.reload();
								}
